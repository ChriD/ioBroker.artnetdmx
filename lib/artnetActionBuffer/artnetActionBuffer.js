...
class ArtnetActionBuffer extends EventEmitter
{
    ...
    startBufferUpdate()
    {
        ...
        // be sure to transmit the artnet buffer one time, because the startBufferUpdateInterval will only 
        // transmit if there was some chnage in the action buffer which is not true on prepared default 
        // buffer!
        this.updateArtnetBuffer(true);
    ...
    updateArtnetBuffer(_forceUpdate = false)
    {
        if(!this.artnetSender)
            return;

        const keys = Object.keys(this.bufferAction);
        if(keys.length === 0 && !_forceUpdate)
            return;
