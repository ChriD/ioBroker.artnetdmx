...
class ArtnetActionBuffer extends EventEmitter
{
    constructor(_configuration)
    {
        ...
        this.bufferExecutionRunning = false;
    ...
    // the buffer has to be updated on interval, if there is something to do, the changes
    ...
    startBufferUpdate()
    {
        ...
        // there buffer may haven been be set externaly, so prepare the buffer channels so the first
        ...
        this.startBufferUpdateInterval();
    }

    startBufferUpdateInterval()
    {
        if(this.bufferUpdateInterval && !this.bufferExecutionRunning)
        {
            this.bufferExecutionRunning = true;
    ...
    updateArtnetBuffer()
    {
        ...
        // if the buffer is empty we do not need to have the interval running, that would only
        //  lead to unneccesary cpu load
        if(!this.bufferAction.length)
        {
            this.stopBufferUpdate();
            return;
        }
    ...
    addAction(_data)
    {
        ...
        // restart the buffer update interval if it's not running
        this.startBufferUpdateInterval();
